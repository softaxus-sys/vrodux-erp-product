...
internal sealed class GetClaimsHandler(CrmDbContext db)
    : IQueryHandler<GetClaimsQuery, PagedResult<InsuranceClaimDto>>
{
    /// <summary>Capped so a hand-edited pageSize cannot ask for the whole table.</summary>
    private const int MaxPageSize = 200;

    public async Task<Result<PagedResult<InsuranceClaimDto>>> Handle(GetClaimsQuery query, CancellationToken ct)
    {
        ...
        var q = db.InsuranceClaims.AsNoTracking().Where(x => !x.IsDeleted);

        if (!string.IsNullOrWhiteSpace(query.Status))
            q = q.Where(x => x.Status == query.Status);

        if (!string.IsNullOrWhiteSpace(query.Search))
            q = q.Where(x => x.ClaimNumber.Contains(query.Search)
                          || x.PolicyNumber.Contains(query.Search)
        ...
        return Result.Success(PagedResult<InsuranceClaimDto>.Create(
