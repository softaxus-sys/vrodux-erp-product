...
internal sealed class GetPatientsHandler(CrmDbContext db)
    : IQueryHandler<GetPatientsQuery, PagedResult<PatientDto>>
{
    ...
    public async Task<Result<PagedResult<PatientDto>>> Handle(GetPatientsQuery query, CancellationToken ct)
    {
        ...
        var q = db.Patients.AsNoTracking().Where(x => !x.IsDeleted);

        if (!string.IsNullOrWhiteSpace(query.Status))
            q = q.Where(x => x.Status == query.Status);

        if (!string.IsNullOrWhiteSpace(query.Search))
            q = q.Where(x => x.PatientNumber.Contains(query.Search)
        ...
        return Result.Success(PagedResult<PatientDto>.Create(
