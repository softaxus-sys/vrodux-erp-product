...
internal sealed class GetTicketsHandler(CrmDbContext db)
    : IQueryHandler<GetTicketsQuery, PagedResult<SupportTicketDto>>
{
    /// <summary>Capped so a hand-edited pageSize cannot ask for the whole table.</summary>
    private const int MaxPageSize = 200;

    public async Task<Result<PagedResult<SupportTicketDto>>> Handle(GetTicketsQuery query, CancellationToken ct)
    {
        ...
        var q = db.SupportTickets.AsNoTracking().Where(x => !x.IsDeleted);

        if (!string.IsNullOrWhiteSpace(query.Status))
            q = q.Where(x => x.Status == query.Status);

        if (!string.IsNullOrWhiteSpace(query.Search))
            q = q.Where(x => x.TicketNumber.Contains(query.Search)
                          || x.ClientName.Contains(query.Search)
                          || x.Subject.Contains(query.Search));
        ...
        return Result.Success(PagedResult<SupportTicketDto>.Create(
