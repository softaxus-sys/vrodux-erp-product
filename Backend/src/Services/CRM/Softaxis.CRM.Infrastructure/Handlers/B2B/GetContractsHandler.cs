...
internal sealed class GetContractsHandler(CrmDbContext db)
    : IQueryHandler<GetContractsQuery, PagedResult<ServiceContractDto>>
{
    /// <summary>Capped so a hand-edited pageSize cannot ask for the whole table.</summary>
    private const int MaxPageSize = 200;

    public async Task<Result<PagedResult<ServiceContractDto>>> Handle(GetContractsQuery query, CancellationToken ct)
    {
        ...
        var q = db.ServiceContracts.AsNoTracking().Where(x => !x.IsDeleted);

        if (!string.IsNullOrWhiteSpace(query.Status))
            q = q.Where(x => x.Status == query.Status);

        if (!string.IsNullOrWhiteSpace(query.Search))
            q = q.Where(x => x.ContractNumber.Contains(query.Search)
        ...
        return Result.Success(PagedResult<ServiceContractDto>.Create(
