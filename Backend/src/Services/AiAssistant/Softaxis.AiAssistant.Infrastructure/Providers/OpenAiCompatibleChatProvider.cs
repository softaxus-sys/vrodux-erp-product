...
/// <summary>
/// Shared implementation for any provider speaking the OpenAI Chat Completions wire format
/// (Groq, OpenRouter, and any future one) — same request/response shape, same tool-calling
/// convention. Subclasses supply only the endpoint URL and which <see cref="AiProvider"/> they are.
/// </summary>
public abstract class OpenAiCompatibleChatProvider(IHttpClientFactory httpClientFactory) : IAiChatProvider
{
    ...
    public async Task<AiCompletionResult> CompleteAsync(AiCompletionRequest request, CancellationToken ct)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new AiProviderException($"{Provider} API error ({(int)response.StatusCode}): {Truncate(payload)}", (int)response.StatusCode);
