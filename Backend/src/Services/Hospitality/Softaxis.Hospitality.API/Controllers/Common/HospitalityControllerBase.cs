...
namespace Softaxis.Hospitality.API.Controllers.Common;

/// <summary>
/// Base controller for all Hospitality API controllers.
/// Converts Result/Result&lt;T&gt; to the appropriate HTTP response without
/// littering every action with if/else boilerplate.
/// </summary>
[ApiController]
public abstract class HospitalityControllerBase : ControllerBase
{
    ...
    /// <summary>201 Created (no route) with the result value, or an error response.</summary>
    protected IActionResult CreatedOrError<T>(Result<T> result) =>
        result.IsSuccess ? StatusCode(StatusCodes.Status201Created, result.Value) : ErrorResponse(result.Error);
