...
internal sealed class SetOrderTipHandler(RestaurantDbContext db)
    : ICommandHandler<SetOrderTipCommand, OrderDto>
{
    public Task<Result<OrderDto>> Handle(SetOrderTipCommand cmd, CancellationToken ct) =>
        ConcurrencyRetry.ExecuteAsync(db, () => HandleOnce(cmd, ct));

    private async Task<Result<OrderDto>> HandleOnce(SetOrderTipCommand cmd, CancellationToken ct)
    {
        ...
        if (order.Status is "paid" or "cancelled" or "split" or "held")
            return Result.Failure<OrderDto>(Error.Custom("Order.Closed", "Cannot change the tip on this order right now."));

        order.SetTip(cmd.Amount);
