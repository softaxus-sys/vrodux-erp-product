...
internal sealed class FireNextCourseHandler(RestaurantDbContext db, IRestaurantRealtimeNotifier realtime)
    : ICommandHandler<FireNextCourseCommand, OrderDto>
{
    public Task<Result<OrderDto>> Handle(FireNextCourseCommand cmd, CancellationToken ct) =>
        ConcurrencyRetry.ExecuteAsync(db, () => HandleOnce(cmd, ct));

    private async Task<Result<OrderDto>> HandleOnce(FireNextCourseCommand cmd, CancellationToken ct)
    {
