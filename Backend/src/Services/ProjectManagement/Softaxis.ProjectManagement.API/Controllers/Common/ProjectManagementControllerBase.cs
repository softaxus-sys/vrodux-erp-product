...
namespace Softaxis.ProjectManagement.API.Controllers.Common;

/// <summary>
/// Base controller for all Project Management API controllers.
...
public abstract class ProjectManagementControllerBase : ControllerBase
{
    ...
    private ObjectResult ErrorResponse(Error error)
    {
        ...
        return error.Code switch
        {
            ...
            var c when c.EndsWith(".HasIssues")      => Conflict(body),
            var c when c.EndsWith(".Conflict")       => Conflict(body),
            var c when c.EndsWith(".AlreadyActive")  => Conflict(body),
