...
internal sealed class GetSuppliersHandler(FinanceDbContext db)
    : IQueryHandler<GetSuppliersQuery, IReadOnlyList<SupplierDto>>
{
    public async Task<Result<IReadOnlyList<SupplierDto>>> Handle(GetSuppliersQuery q, CancellationToken ct)
    {
        ...
        return Result.Success<IReadOnlyList<SupplierDto>>(items);
    }
}
