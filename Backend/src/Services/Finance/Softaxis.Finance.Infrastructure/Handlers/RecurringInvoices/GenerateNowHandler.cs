...
internal sealed class GenerateNowHandler(FinanceDbContext db) : ICommandHandler<GenerateNowCommand, GenerateInvoiceResultDto>
{
    public async Task<Result<GenerateInvoiceResultDto>> Handle(GenerateNowCommand cmd, CancellationToken ct)
    {
        ...
        return Result.Success(new GenerateInvoiceResultDto(invoice.Id, invoice.InvoiceNumber));
    }
}
