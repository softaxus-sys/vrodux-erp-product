...
import { useBrokers } from "@/hooks/real-estate/use-real-estate";
import type { BrokerDto as Broker, BrokerStatus } from "@/lib/real-estate/real-estate.api";
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
          ...
          <div className="flex items-start justify-between px-6 py-5 border-b border-border">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{broker.agency}</p>
              <p className="text-xs font-mono text-muted-foreground">{broker.brokerCode} · License: {broker.licenseNumber}</p>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="bg-muted/30 rounded-xl p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">License Expiry</span><span className="font-semibold">{broker.licenseExpiry}</span></div>
            </div>
          </div>
...
export function BrokersView() {
  ...
  const totalDeals        = brokers.reduce((s, b) => s + b.closedDeals, 0);
  const totalCommission   = brokers.reduce((s, b) => s + b.totalCommission, 0);
  const avgRating         = brokers.length
    ? Math.round((brokers.reduce((s, b) => s + b.rating, 0) / brokers.length) * 10) / 10
    : 0;

  const STATS = [
    { label: "Total Brokers",     value: brokers.length,                          icon: UserCheck,  color: "text-slate-600", bg: "bg-slate-100 dark:bg-slate-800/50" },
    { label: "Active",            value: activeCount,                             icon: BadgeCheck, color: "text-success",   bg: "bg-success/10" },
    { label: "Total Deals",       value: totalDeals,                              icon: TrendingUp, color: "text-primary",   bg: "bg-primary/10" },
    { label: "Total Commission",  value: formatCurrency(totalCommission, "AED"),  icon: DollarSign, color: "text-success",   bg: "bg-success/10", isText: true },
  ...
  const filtered = React.useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(b =>
        ...
        b.brokerCode.toLowerCase().includes(s) ||
        b.licenseNumber.toLowerCase().includes(s)
      );
    }
    return list;
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden lg:table-cell">License</th>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <td className="px-4 py-3.5 text-sm text-muted-foreground hidden md:table-cell">{b.agency}</td>
                  <td className="px-4 py-3.5 text-xs font-mono text-muted-foreground hidden lg:table-cell">{b.licenseNumber}</td>
