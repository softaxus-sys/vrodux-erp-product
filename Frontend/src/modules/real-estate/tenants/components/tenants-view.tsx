...
import { useTenants } from "@/hooks/real-estate/use-real-estate";
...
// STAT_CARDS computed inside component
...
export function TenantsView() {
  ...
  const corporateCount = tenants.filter(t => t.type === "corporate").length;
  const individualCount = tenants.filter(t => t.type === "individual").length;
  const totalRent      = tenants.reduce((s, t) => s + t.totalRentPA, 0);
  const avgPaymentScore = tenants.length
    ? Math.round((tenants.reduce((s, t) => {
        const scores: Record<string, number> = { excellent: 4, good: 3, fair: 2, poor: 1 };
        return s + (scores[t.paymentHistory] ?? 0);
      }, 0) / tenants.length) * 10) / 10
    : 0;

  const STAT_CARDS = [
    { label: "Total Tenants", value: tenants.length.toString(),              icon: Users,       color: "text-primary bg-primary/10" },
    { label: "Active",        value: activeCount.toString(),                 icon: UserCheck,   color: "text-success bg-success/10" },
    { label: "Corporate",     value: corporateCount.toString(),              icon: Building2,   color: "text-blue-600 bg-blue-500/10" },
    { label: "Individual",    value: individualCount.toString(),             icon: User,        color: "text-purple-600 bg-purple-500/10" },
    { label: "Total Rent PA", value: formatCurrency(totalRent, "AED"),       icon: AlertTriangle,color: "text-destructive bg-destructive/10" },
    { label: "Avg Rating",    value: `${avgPaymentScore}/4`,                 icon: Star,        color: "text-warning bg-warning/10" },
  ];
  ...
  }, [tenants, search, typeFilter, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <TenantsDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} tenant={selected} />
