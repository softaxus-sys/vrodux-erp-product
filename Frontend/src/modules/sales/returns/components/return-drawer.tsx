...
import {
  X, CheckCircle2, Ban, Clock, RefreshCw,
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className={cn("border rounded-xl p-5",
                ...
                <p className={cn("text-3xl font-bold",
                  ret.status === "refunded" || ret.status === "completed" ? "text-success" : "text-foreground")}>
            ...
            <div className="border-t border-border px-6 py-4 flex items-center gap-2">
              {ret.status === "pending" && (
                <>
                  <Button size="sm" className="gap-1.5 h-9 bg-success hover:bg-success/90"
                    ...
                    onClick={() => approveReturn.mutate({ id: ret.id, refundMethod: "bank_transfer" })}>
                  ...
                  <Button variant="outline" size="sm" className="gap-1.5 h-9 text-destructive border-destructive/30 hover:bg-destructive/10"
                    ...
                    onClick={() => rejectReturn.mutate({ id: ret.id, reason: "Rejected by reviewer" })}>
              ...
              {ret.status === "approved" && (
                <Button size="sm" className="gap-1.5 h-9"
                  disabled={approveReturn.isPending}
                  onClick={() => approveReturn.mutate({ id: ret.id, refundMethod: ret.refundMethod ?? "bank_transfer" })}>
