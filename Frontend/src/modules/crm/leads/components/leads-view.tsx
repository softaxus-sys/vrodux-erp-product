...
import { useLeads, useLeadSummary } from "@/hooks/crm/use-leads";
import {
  SOURCE_LABELS,
...
/* ── Kanban card ── */
function LeadKanbanCard({ lead, index, onClick }: { lead: LeadDto; index: number; onClick: () => void }) {
...
/* ── Kanban board ── */
function LeadsKanban({ leads, onLeadClick }: { leads: LeadDto[]; onLeadClick: (l: LeadDto) => void }) {
  return (
...
export function LeadsView() {
  ...
  const openDrawer = (l: LeadDto) => { setSelectedLead(l); setDrawerOpen(true); };

  const uniqueSources = [...new Set(leads.map(l => l.source))] as LeadSource[];

  const totalCount      = summary?.total                ?? leads.length;
  const newCount        = summary?.newThisWeek          ?? 0;
  const qualifiedCount  = summary?.qualified            ?? 0;
  const contactedCount  = summary?.contacted            ?? 0;
  const convertedCount  = summary?.converted            ?? 0;
  const conversionRate  = summary?.conversionRate       ?? 0;
  const pipelineValue   = summary?.totalEstimatedValue  ?? 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="h-9 gap-1.5 text-sm" onClick={() => setShowAddForm(true)}><Plus className="h-4 w-4" />Add Lead</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        {[
          { label: "Total Leads",    value: totalCount,                              sub: "All time",        icon: Users,        color: "text-primary bg-primary/10" },
          { label: "New",            value: newCount,                                sub: "Needs contact",   icon: Zap,          color: "text-slate-600 bg-slate-100 dark:bg-slate-800/50" },
          { label: "Qualified",      value: qualifiedCount,                          sub: "Hot leads",       icon: Target,       color: "text-success bg-success/10" },
          { label: "Contacted",      value: contactedCount,                          sub: "In follow-up",    icon: TrendingUp,   color: "text-blue-600 bg-blue-100 dark:bg-blue-900/20" },
          { label: "Converted",      value: convertedCount,                          sub: "To deals",        icon: CheckCircle2, color: "text-violet-600 bg-violet-100 dark:bg-violet-900/20" },
          { label: "Pipeline Value", value: formatCurrency(pipelineValue, "AED"),    sub: "Est. value",      icon: DollarSign,   color: "text-warning bg-warning/10" },
        ].map((s, i) => (
      ...
      {/* Kanban */}
      {!isLoading && viewMode === "kanban" && <LeadsKanban leads={filtered} onLeadClick={openDrawer} />}
        ...
        <Card>
          <CardContent className="p-0">
            ...
            <div className="px-4 py-3 border-t border-border text-xs text-muted-foreground">
              Showing {filtered.length} of {leads.length} leads · Conversion rate: {conversionRate}%
            </div>
          </CardContent>
        </Card>
      )}
