...
import type { CustomerDto, CustomerActivityDto } from "@/lib/crm/customers.api";
...
const ACTIVITY_ICON: Record<CustomerActivityDto["type"], React.ElementType> = {
  call: PhoneCall, email: Mail, meeting: Users, note: FileText, support: MessageSquare,
};
const ACTIVITY_COLOR: Record<CustomerActivityDto["type"], string> = {
  ...
  support: "bg-pink-100 text-pink-600 dark:bg-pink-900/30",
};
...
interface Props { customer: CustomerDto | null; open: boolean; onClose: () => void; }

export function CustomerDrawer({ customer, open, onClose }: Props) {
        ...
        <>
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
              ...
              {tab === "contacts" && (
                <div className="p-6 space-y-3">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-sm font-semibold">Contacts ({customer.contacts.length})</h4>
                    <Button size="sm" className="h-8 text-xs gap-1.5"><User className="h-3.5 w-3.5" />Add Contact</Button>
                  </div>
                  {customer.contacts.map((c, i) => (
                    <motion.div key={c.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      <Avatar className="h-10 w-10 shrink-0">
                        <AvatarFallback className="text-xs font-bold bg-primary/10 text-primary">{getInitials(c.name)}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-sm">{c.name}</p>
                          {c.isPrimary && (
                            <span className="px-1.5 py-0.5 bg-primary/10 text-primary text-[10px] font-semibold rounded">Primary</span>
                          )}
                        </div>
              ...
              {tab === "activity" && (
                <div className="p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <Button size="sm" className="h-8 text-xs gap-1.5"><MessageSquare className="h-3.5 w-3.5" />Log Activity</Button>
                  </div>
                  <div className="relative">
                    <div className="absolute left-5 top-0 bottom-0 w-px bg-border" />
                    <div className="space-y-4">
                      {[...customer.activities].reverse().map((act, i) => {
            ...
            <div className="border-t border-border px-6 py-4 flex items-center gap-2">
              <Button size="sm" className="gap-1.5 h-9"><PhoneCall className="h-3.5 w-3.5" />Log Call</Button>
              <Button variant="outline" size="sm" className="gap-1.5 h-9"><Mail className="h-3.5 w-3.5" />Send Email</Button>
              <Button variant="outline" size="sm" className="gap-1.5 h-9"><Users className="h-3.5 w-3.5" />Schedule Meeting</Button>
            </div>
          </motion.div>
        </>
      )}
