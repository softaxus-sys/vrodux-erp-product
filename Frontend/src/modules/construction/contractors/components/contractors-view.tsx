...
import { useContractors } from "@/hooks/construction/use-construction";
import type { ContractorDto, ContractorStatus, ContractorTrade } from "@/lib/construction/construction.api";
...
function ContractorDrawer({ contractor, open, onClose }: { contractor: ContractorDto | null; open: boolean; onClose: () => void }) {
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="bg-muted/30 rounded-xl p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Location</span><span>{contractor.locationCity}, {contractor.locationCountry}</span></div>
            </div>
            <div className="bg-muted/30 rounded-xl p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Insurance Provider</span><span>{contractor.insuranceProvider}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Insurance Expiry</span><span>{formatDate(contractor.insuranceExpiry, "medium")}</span></div>
            </div>
...
export function ContractorsView() {
  ...
  const [selected, setSelected] = React.useState<ContractorDto | null>(null);
  ...
  const totalValue   = contractors.reduce((s, c) => s + c.totalContractValue, 0);
  ...
  const STATS = [
    { label: "Total",       value: contractors.length,                         icon: HardHat,    color: "text-slate-600", bg: "bg-slate-100 dark:bg-slate-800/50" },
    { label: "Active",      value: activeCount,                                icon: CheckCircle2,color: "text-success",  bg: "bg-success/10" },
    { label: "Blacklisted", value: blacklisted,                                icon: Ban,         color: "text-destructive",bg: "bg-destructive/10" },
    { label: "Avg Rating",  value: `${avgRating}★`,                            icon: Star,        color: "text-warning",  bg: "bg-warning/10" },
    { label: "Total Value", value: formatCurrency(totalValue, "AED"),          icon: Shield,      color: "text-primary",  bg: "bg-primary/10", isText: true },
  ];
                ...
                <motion.tr key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <td className="px-4 py-3.5 hidden lg:table-cell">
                    <span className={cn("text-sm", licWarning ? "text-warning font-semibold" : "text-muted-foreground")}>
                      {formatDate(c.licenseExpiry, "short")}
                    </span>
