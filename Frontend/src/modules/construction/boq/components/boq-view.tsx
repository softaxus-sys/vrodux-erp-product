...
import { useBOQs } from "@/hooks/construction/use-construction";
import type { BOQDto, BOQStatus } from "@/lib/construction/construction.api";
...
function BOQDrawer({ boq, open, onClose }: { boq: BOQDto | null; open: boolean; onClose: () => void }) {
...
export function BOQView() {
  ...
  const [selected, setSelected] = React.useState<BOQDto | null>(null);
  ...
  const totalValue = boqs.reduce((s, b) => s + b.totalAmount, 0);

  const STATS = [
    { label: "Total BOQs",   value: boqs.length,                                                               icon: FileText,   color: "text-slate-600", bg: "bg-slate-100 dark:bg-slate-800/50" },
    { label: "In Progress",  value: boqs.filter(b => b.status === "in_progress").length,                       icon: TrendingUp, color: "text-warning",   bg: "bg-warning/10" },
    { label: "Completed",    value: boqs.filter(b => b.status === "completed").length,                         icon: CheckCircle2,color: "text-success",  bg: "bg-success/10" },
    { label: "Total Value",  value: formatCurrency(totalValue, "AED"),                                         icon: DollarSign, color: "text-primary",   bg: "bg-primary/10", isText: true },
  ];
