...
import { useTransfers, useTransferSummary, useApproveTransfer, useReceiveTransfer } from "@/hooks/inventory/use-transfers";
import type { StockTransferDto, TransferStatus } from "@/lib/inventory/transfers.api";
import { AddTransferForm } from "./add-transfer-form";

const STATUS_CONFIG: Record<TransferStatus, { label: string; color: string; bg: string; dot: string }> = {
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
          ...
          <div className="border-t border-border px-6 py-4 flex items-center gap-2">
            {transfer.status === "draft" && (
              <Button size="sm" className="gap-1.5 h-9" onClick={() => approve.mutate(transfer.id)} disabled={approve.isPending}>
                <CheckCircle2 className="h-3.5 w-3.5" />Approve
              </Button>
            )}
            {transfer.status === "in_transit" && (
              <Button size="sm" className="gap-1.5 h-9 bg-success hover:bg-success/90" onClick={() => receive.mutate(transfer.id)} disabled={receive.isPending}>
...
export function TransfersView() {
  ...
  const [selected, setSelected] = React.useState<StockTransferDto | null>(null);
  const [drawerOpen, setDrawerOpen] = React.useState(false);
  const [showAddForm, setShowAddForm] = React.useState(false);

  const { data: paged, isLoading } = useTransfers(
    statusFilter !== "all" ? { status: statusFilter } : undefined
  );
  const { data: summary } = useTransferSummary();

  const transfers = paged?.items ?? [];
  ...
  const filtered = React.useMemo(() => {
    ...
    return transfers.filter(
      (t) =>
        ...
        t.fromWarehouseName.toLowerCase().includes(s) ||
        t.toWarehouseName.toLowerCase().includes(s)
    );
  }, [transfers, search]);

  const STATS = [
    { label: "Total",      value: summary?.total ?? 0,      icon: ArrowLeftRight, color: "text-slate-600", bg: "bg-slate-100 dark:bg-slate-800/50" },
    { label: "Pending",    value: summary?.pending ?? 0,    icon: Clock,          color: "text-warning",   bg: "bg-warning/10" },
    { label: "In Transit", value: summary?.inTransit ?? 0,  icon: Truck,          color: "text-primary",   bg: "bg-primary/10" },
    { label: "Received",   value: summary?.received ?? 0,   icon: CheckCircle2,   color: "text-success",   bg: "bg-success/10" },
    {
      label: "Total Value",
      value: formatCurrency(summary?.totalValue ?? 0, "AED"),
      icon: DollarSign,
      color: "text-success",
      bg: "bg-success/10",
      isText: true,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Move inventory between warehouses and locations</p>
        </div>
                ...
                <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <td className="px-4 py-3.5 hidden lg:table-cell">
                    <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                      <Calendar className="h-3.5 w-3.5" />{formatDate(t.expectedDate, "short")}
                    </div>
                  </td>
      ...
      <TransferDrawer transfer={selected} open={drawerOpen} onClose={() => setDrawerOpen(false)} />
