...
import { useBookings, useBookingSummary, useCheckIn, useCheckOut } from "@/hooks/hospitality/use-hospitality";
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
          ...
          <div className="flex items-start justify-between px-6 py-5 border-b border-border">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{booking.guestNationality} · Room {booking.roomNumber} ({ROOM_TYPE_LABELS[booking.roomType] ?? booking.roomType})</p>
          ...
          <div className="border-t border-border px-6 py-4 flex items-center gap-2">
            {booking.status === "confirmed" && (
              <Button size="sm" className="gap-1.5 h-9" disabled={checkIn.isPending}
                onClick={() => checkIn.mutate(booking.id)}>
                <LogIn className="h-3.5 w-3.5" />Check In
              </Button>
            )}
            {booking.status === "checked_in" && (
              <Button size="sm" className="gap-1.5 h-9 bg-success hover:bg-success/90" disabled={checkOut.isPending}
...
export function BookingsView() {
  ...
  const outstandingBalance  = bookings.reduce((s, b) => s + b.balance, 0);

  const STATS = [
    { label: "Total Bookings", value: summary?.total        ?? bookings.length, icon: CalendarCheck, color: "text-slate-600", bg: "bg-slate-100 dark:bg-slate-800/50" },
    { label: "Checked In",     value: summary?.checkedIn    ?? checkedInCount,  icon: BedDouble,     color: "text-success",   bg: "bg-success/10" },
    { label: "Arriving Today", value: summary?.confirmed    ?? confirmedCount,  icon: LogIn,         color: "text-primary",   bg: "bg-primary/10" },
    { label: "Occupancy",      value: `${summary?.occupancyRate ?? 0}%`,        icon: Users,         color: "text-warning",   bg: "bg-warning/10" },
    { label: "Revenue",        value: formatCurrency(summary?.totalRevenue ?? totalRevenue, "AED"), icon: DollarSign, color: "text-success", bg: "bg-success/10", isText: true },
  ];
    ...
    <div className="space-y-6">
      ...
      {outstandingBalance > 0 && (
        <div className="flex items-center gap-2 bg-warning/10 border border-warning/30 rounded-xl px-4 py-3 text-sm text-warning">
          ...
          <span>Outstanding balance of <strong>{formatCurrency(outstandingBalance, "AED")}</strong> across active bookings.</span>
        </div>
      )}
              ...
              return (
                <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <td className="px-4 py-3.5 hidden md:table-cell">
                    ...
                    <p className="text-xs text-muted-foreground">{ROOM_TYPE_LABELS[b.roomType] ?? b.roomType} · Floor {b.floor}</p>
                  </td>
