...
import {
  useTaxPeriods, useTaxSummary, useTaxTransactions,
  useFileTaxPeriod, usePayTaxPeriod,
} from "@/hooks/finance/use-tax";
import type { TaxPeriodDto } from "@/lib/finance/tax.api";
...
function TaxDrawer({ period, onClose }: { period: TaxPeriodDto; onClose: () => void }) {
  const [tab, setTab]         = React.useState<"overview" | "sales" | "purchases">("overview");
  const { data: transactions = [] } = useTaxTransactions(period.id);
  const filePeriod  = useFileTaxPeriod();
  const payPeriod   = usePayTaxPeriod();

  const sc         = STATUS_CONFIG[period.status] ?? { label: period.status, color: "text-foreground", bg: "bg-muted", dot: "bg-muted-foreground" };
  const salesTxns  = transactions.filter(t => t.type === "sale");
  const purchTxns  = transactions.filter(t => t.type === "purchase");
    ...
    <AnimatePresence>
      ...
      <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
        ...
        <div className="flex border-b border-border px-6">
          {(["overview", "sales", "purchases"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "sales" ? `Sales (${salesTxns.length})` : t === "purchases" ? `Purchases (${purchTxns.length})` : "Overview"}
            </button>
          ))}
        </div>
            ...
            <>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className={cn("border rounded-xl p-4 text-center", period.netVat >= 0 ? "bg-warning/5 border-warning/20" : "bg-success/5 border-success/20")}>
                  ...
                  <p className={cn("font-bold text-lg", period.netVat >= 0 ? "text-warning" : "text-success")}>
                    {formatCurrency(Math.abs(period.netVat), "AED")}
                  </p>
              ...
              <div className="bg-muted/30 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Filing Due</span>
                  <span className={cn(period.status === "overdue" ? "text-destructive font-semibold" : "")}>
                    {formatDate(period.dueDate, "medium")}
                  </span>
                </div>
                {period.filedDate && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Filed On</span>
                    <span className="text-success">{formatDate(period.filedDate, "medium")}</span>
                  </div>
                )}
                {period.paidDate && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Paid On</span>
                    <span className="text-success">{formatDate(period.paidDate, "medium")}</span>
                  </div>
                )}
                {period.penalty != null && period.penalty > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Penalty</span>
                    <span className="text-destructive font-semibold">{formatCurrency(period.penalty, "AED")}</span>
                  </div>
                )}
              </div>
            </>
          ...
          {(tab === "sales" || tab === "purchases") && (
            <div className="rounded-xl border border-border overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(tab === "sales" ? salesTxns : purchTxns).map((t, i) => (
                    <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      transition={{ delay: i * 0.03 }} className="border-t border-border/40 hover:bg-muted/20">
                      <td className="px-4 py-3 text-muted-foreground text-xs">{formatDate(t.date, "short")}</td>
                      <td className="px-4 py-3">
                        <p className="text-sm font-medium">{t.description}</p>
                        {t.reference && <p className="text-xs text-muted-foreground font-mono">{t.reference}</p>}
                      </td>
                  ...
                  {(tab === "sales" ? salesTxns : purchTxns).length === 0 && (
                    <tr><td colSpan={4} className="text-center py-8 text-sm text-muted-foreground">No transactions.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="border-t border-border px-6 py-4 flex items-center gap-2">
          {period.status === "open" && (
            <Button size="sm" className="gap-1.5 h-9" disabled={filePeriod.isPending}
              onClick={() => { filePeriod.mutate(period.id); onClose(); }}>
              <FileText className="h-3.5 w-3.5" /> File Return
            </Button>
          )}
          {period.status === "filed" && (
            <Button size="sm" className="gap-1.5 h-9 bg-success hover:bg-success/90"
              disabled={payPeriod.isPending}
              onClick={() => { payPeriod.mutate(period.id); onClose(); }}>
...
export function TaxView() {
  const [selected, setSelected]   = React.useState<TaxPeriodDto | null>(null);

  const { data: periods = [], isLoading } = useTaxPeriods();
  const { data: summary } = useTaxSummary();

  const STATS = [
    { label: "Output VAT (Current)", value: formatCurrency(summary?.currentPeriodOutput ?? 0, "AED"), icon: ArrowUpRight,  color: "text-success",     bg: "bg-success/10" },
    { label: "Input VAT (Current)",  value: formatCurrency(summary?.currentPeriodInput  ?? 0, "AED"), icon: ArrowDownRight, color: "text-primary",    bg: "bg-primary/10" },
    { label: "Net VAT Payable",      value: formatCurrency(summary?.currentNetVat        ?? 0, "AED"), icon: Receipt,       color: "text-warning",     bg: "bg-warning/10" },
    { label: "YTD VAT Paid",         value: formatCurrency(summary?.ytdVatPaid           ?? 0, "AED"), icon: CheckCircle2,  color: "text-success",     bg: "bg-success/10" },
    { label: "Next Filing Due",      value: summary?.nextDueDate ? formatDate(summary.nextDueDate, "short") : "—", icon: Calendar, color: "text-destructive", bg: "bg-destructive/10" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Tax & VAT</h1>
          <p className="text-sm text-muted-foreground mt-0.5">UAE VAT returns, filing, and tax transaction tracking</p>
        </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="text-center py-12 text-sm text-muted-foreground">No tax periods found.</td></tr>
            ) : periods.map((p: TaxPeriodDto, i) => {
              const sc = STATUS_CONFIG[p.status] ?? { label: p.status, color: "text-foreground", bg: "bg-muted", dot: "bg-muted-foreground" };
              return (
                <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                  ...
                  <td className="px-4 py-4 text-sm text-muted-foreground hidden md:table-cell">
                    {formatDate(p.from, "short")} — {formatDate(p.to, "short")}
                  </td>
                  ...
                  <td className="px-4 py-4 text-sm text-muted-foreground hidden md:table-cell">
                    <span className={cn(p.status === "overdue" ? "text-destructive font-semibold" : "")}>
                      {formatDate(p.dueDate, "short")}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right" onClick={e => e.stopPropagation()}>
                    {p.status === "open"  && <Button size="sm" className="h-7 text-xs">File</Button>}
                    {p.status === "filed" && <Button size="sm" className="h-7 text-xs bg-success hover:bg-success/90">Pay</Button>}
                  </td>
                </motion.tr>
              );
            })}
      ...
      <AnimatePresence>
        {selected && <TaxDrawer period={selected} onClose={() => setSelected(null)} />}
      </AnimatePresence>
    </div>
  );
}
