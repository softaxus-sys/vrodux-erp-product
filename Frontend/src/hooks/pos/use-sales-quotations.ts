...
} from "@/lib/pos/sales-quotations.api";
...
export const salesQuotationKeys = {
  all:     ["pos-sales-quotations"] as const,
...
export function useCreateSalesQuotation() {
  ...
  return useMutation({
    mutationFn: (payload: CreateQuotationPayload) => salesQuotationsApi.create(payload),
    onSuccess: () => {
...
export function useUpdateSalesQuotation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...payload }: { id: string } & UpdateQuotationPayload) =>
