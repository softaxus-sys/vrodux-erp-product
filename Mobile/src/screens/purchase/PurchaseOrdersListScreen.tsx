    ...
    <View style={styles.container}>
      ...
      {query.isError ? (
        <View style={styles.centered}>
          <Text style={styles.errorText}>Couldn&apos;t load purchase orders.</Text>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No purchase orders here.</Text>}
          ListFooterComponent={
...
function OrderRow({ order, currency, onPress }: { order: PurchaseOrderSummaryDto; currency: string; onPress: () => void }) {
  return (
    <Pressable style={({ pressed }) => [styles.row, pressed && styles.rowPressed]} onPress={onPress}>
      ...
      <Text style={styles.status}>{PURCHASE_ORDER_STATUS_LABELS[order.status] ?? order.status}</Text>
    </Pressable>
  );
}
