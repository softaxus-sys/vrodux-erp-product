...
export function AddStockItemForm({ open, onClose, editingId }: AddStockItemFormProps) {
  ...
  const { mutate: saveProduct, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(isEdit ? "Stock item updated." : "Stock item created successfully.");
      handleClose();
    },
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              <div>
                <h2 className="text-base font-bold text-foreground">{isEdit ? "Edit Stock Item" : "New Stock Item"}</h2>
                <p className="text-xs text-muted-foreground mt-0.5">{isEdit ? "Update product details" : "Add a new item to the inventory catalog"}</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {autofill.status !== "idle" && (
                    <div className="col-span-2 -mt-1">
                      <InventoryBarcodeStatusBanner autofill={autofill} />
                    </div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" min={0} step={1} value={isEdit ? (editingProduct?.stockQuantity ?? "") : openingQty}
                      ...
                      disabled={isEdit}
                      title={isEdit ? "Use a stock adjustment to change on-hand quantity" : undefined}
            ...
            <div className="px-6 py-4 border-t border-border flex gap-2 justify-between shrink-0">
              ...
              <Button onClick={() => saveProduct()} disabled={!isValid || isPending}>
                {isPending ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />Saving…</> : (isEdit ? "Save Changes" : "Save Item")}
              </Button>
            </div>
          </motion.div>
        </>
...
function InventoryBarcodeStatusBanner({ autofill }: { autofill: ReturnType<typeof useBarcodeAutofill> }) {
  ...
  if (autofill.status === "found" && autofill.data) {
    ...
    return (
      <div className="flex items-start gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 dark:bg-emerald-900/20 dark:border-emerald-700 dark:text-emerald-400">
        ...
        <p className="text-[11px] leading-snug">
          <span className="font-semibold">Auto-filled</span> from {AUTOFILL_SOURCE_LABELS[data.source]}
        </p>
      </div>
    );
  }
  ...
  if (autofill.status === "not-found") {
    return (
      <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-muted/60 border border-border text-muted-foreground">
        ...
        <p className="text-[11px]">Barcode not found in any database. Fill details manually.</p>
      </div>
    );
  }
