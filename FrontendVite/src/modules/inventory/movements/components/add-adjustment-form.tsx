...
import { useCreateStockMovement } from "@/hooks/inventory/use-stock-movements";
import { MovementType, type ProductSummaryDto } from "@/lib/inventory/types";
...
export function AddAdjustmentForm({ open, onClose, preselectedItemId }: AddAdjustmentFormProps) {
  ...
  const filteredItems = React.useMemo(() => {
    if (!itemSearch.trim()) return allProducts.slice(0, 10);
  ...
  const handleSubmit = () => {
    ...
    // Map the UI adjustment type → backend movement type + signed quantity.
    let movementType: string;
    let signedQty: number;
    switch (adjType) {
      case "in":        movementType = MovementType.Receipt;    signedQty =  qty; break;
      case "write_off": movementType = MovementType.WriteOff;   signedQty =  qty; break; // server negates
      default:          movementType = MovementType.Adjustment; signedQty = direction === "-" ? -qty : qty; break;
    }
    ...
    createMovement.mutate(
      {
        ...
        movementType,
        quantity:     signedQty,
        unitCost:     selectedItem.costPrice ?? 0,
        reference:    reference.trim() || null,
        notes:        [reason, notes].filter(Boolean).join(" — ") || null,
        warehouseId:  warehouseId || null,
        batchNumber:  adjType === "in" && batchNumber.trim() ? batchNumber.trim() : null,
        expiryDate:   adjType === "in" && expiryDate ? expiryDate : null,
      },
      {
        onSuccess: () => {
        ...
        <>
          ...
          <motion.div
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
                          ...
                          {filteredItems.map(item => (
                            <button key={item.id} onMouseDown={() => { setSelectedItem(item); setItemSearch(item.name); setShowItemList(false); }}
                              ...
                              <span className={cn("text-xs px-1.5 py-0.5 rounded font-medium",
                                ...
                                item.isLowStock ? "bg-warning/10 text-warning" :
              ...
              {/* Batch / Expiry — only meaningful when receiving stock in */}
              {adjType === "in" && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
