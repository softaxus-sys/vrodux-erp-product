  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Categories, items, pricing, and modifier groups.</p>
      </div>

      <div className="flex gap-2 border-b border-border pb-2">
        <TabButton active={tab === "menu"} onClick={() => setTab("menu")} icon={Layers}>Menu</TabButton>
        <TabButton active={tab === "modifiers"} onClick={() => setTab("modifiers")} icon={Sliders}>Modifier Groups</TabButton>
      </div>
    ...
    <LeftDrawer onClose={onCancel} widthClassName="max-w-sm" zIndexClassName="z-[60]">
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button variant="destructive" size="sm" onClick={onConfirm} disabled={pending}>
          {pending && <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />} Delete
        </Button>
      </div>
    </LeftDrawer>
  );
    ...
    <div className="space-y-4">
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <StatCard label="Available" value={summary.availableItems} />
          <StatCard label="Unavailable" value={summary.unavailableItems} />
          <StatCard label="Avg Price" value={formatCurrency(summary.avgPrice, currency)} />
          <StatCard label="Price Range" value={`${formatCurrency(summary.minPrice, currency)} – ${formatCurrency(summary.maxPrice, currency)}`} />
        </div>
      )}
      ...
      {totalItemCount > 0 && unlinkedItemCount > 0 && (
        <Link to="/recipe/recipes" className="flex items-center gap-2 rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning hover:bg-warning/15 transition-colors">
          ...
          <span>{unlinkedItemCount} of {totalItemCount} menu items have no recipe linked — stock won't auto-deduct when served. Manage recipes →</span>
        </Link>
      )}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground py-10 text-center">No menu categories yet. Add one to get started.</p>
      ) : (
              ...
              <div key={cat.id} className="bg-card border border-border rounded-xl overflow-hidden">
                ...
                {isOpen && (
                  <div className="border-t border-border">
                    {cat.items.length === 0 ? (
                      <p className="text-sm text-muted-foreground px-4 py-4">No items in this category yet.</p>
                    ) : (
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2">
                                ...
                                {item.modifierGroups.length > 0 && (
                                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary">{item.modifierGroups.length} modifier{item.modifierGroups.length !== 1 ? "s" : ""}</span>
                                )}
                                <Link to="/recipe/recipes" title={linkedMenuItemIds.has(item.id) ? "Recipe linked — stock auto-deducts when served" : "No recipe linked — stock won't auto-deduct when served"}
                              ...
                              <p className="text-xs text-muted-foreground truncate">
                                {formatCurrency(item.price, currency)} · {item.prepTimeMinutes} min
      ...
      {deleteTarget && (
        <ConfirmModal
          title={`Delete ${deleteTarget.kind === "category" ? "category" : "item"}?`}
  ...
  return (
    <LeftDrawer onClose={onClose} widthClassName="max-w-sm">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-foreground">{category ? "Edit" : "Add"} Category</p>
        <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
      <div><label className="text-xs text-muted-foreground">Name</label>
        <Input value={name} onChange={e => setName(e.target.value)} placeholder="Starters" className="h-9 text-sm" /></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div><label className="text-xs text-muted-foreground">Sort Order</label>
          <Input type="number" value={sortOrder} onChange={e => setSortOrder(e.target.value)} className="h-9 text-sm" /></div>
        <div><label className="text-xs text-muted-foreground">Kitchen Station</label>
...
function ItemModal({ item, categoryId, stations, onClose, onSave }: {
  ...
  return (
    <LeftDrawer onClose={onClose} widthClassName="max-w-md">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-foreground">{item ? "Edit" : "Add"} Menu Item</p>
        <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
      <div><label className="text-xs text-muted-foreground">Name</label>
        <Input value={name} onChange={e => setName(e.target.value)} placeholder="Grilled Chicken" className="h-9 text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Description (optional)</label>
        <Input value={description} onChange={e => setDescription(e.target.value)} className="h-9 text-sm" /></div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div><label className="text-xs text-muted-foreground">Prep Time (min)</label>
          <Input type="number" min={0} value={prepTimeMinutes} onChange={e => setPrepTimeMinutes(e.target.value)} className="h-9 text-sm" /></div>
      </div>
      <div><label className="text-xs text-muted-foreground">Allergens (optional)</label>
        <Input value={allergens} onChange={e => setAllergens(e.target.value)} placeholder="Nuts, Dairy" className="h-9 text-sm" /></div>
      <div><label className="text-xs text-muted-foreground">Kitchen Station</label>
        <select value={kitchenStationId} onChange={e => setKitchenStationId(e.target.value)}
          className="w-full h-9 text-sm rounded-md border border-border bg-card px-2">
      ...
      <label className="flex items-center gap-2 text-sm text-foreground">
        ...
        Orderable via QR / kiosk / online
      </label>

      {allGroups.length > 0 && (
        <div>
          <label className="text-xs text-muted-foreground">Modifier Groups</label>
      ...
      <Button className="w-full" disabled={!valid || saving} onClick={handleSave}>
        {saving && <Loader2 className="w-4 h-4 mr-1 animate-spin" />} Save Item
      </Button>
    </LeftDrawer>
  );
}
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Groups like "Size" or "Add-ons" that customers pick from when ordering an item — assign them per item from the Menu tab.
      </p>
      {canCreate && <Button size="sm" onClick={() => setEditing("new")}><Plus className="w-4 h-4 mr-1" /> Add Modifier Group</Button>}

      {isLoading ? (
        <div className="flex items-center justify-center h-40 text-muted-foreground"><Loader2 className="animate-spin mr-2 h-5 w-5" /> Loading…</div>
      ) : groups.length === 0 ? (
        <p className="text-sm text-muted-foreground py-10 text-center">No modifier groups yet.</p>
      ) : (
          ...
          {groups.map(g => (
            <div key={g.id} className="bg-card border border-border rounded-xl p-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {g.minSelect === 0 ? "Optional" : `Requires ${g.minSelect}`} · Max {g.maxSelect}
              </p>
              <div className="flex flex-wrap gap-1 mt-2">
                ...
                {g.modifiers.length === 0 && <span className="text-[11px] text-muted-foreground">No modifiers</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {deleteTarget && (
        <ConfirmModal
          ...
          message={`"${deleteTarget.name}" will be removed from all menu items using it.`}
          onCancel={() => setDeleteTarget(null)}
          onConfirm={handleConfirmDelete}
...
function ModifierGroupModal({ group, onClose, onSave }: {
  ...
  return (
    <LeftDrawer onClose={onClose} widthClassName="max-w-md">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-foreground">{group ? "Edit" : "Add"} Modifier Group</p>
        <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
      <div><label className="text-xs text-muted-foreground">Name</label>
        <Input value={name} onChange={e => setName(e.target.value)} placeholder="Size" className="h-9 text-sm" /></div>
      <div className="grid grid-cols-2 gap-2">
        <div><label className="text-xs text-muted-foreground">Min Select (0 = optional)</label>
          <Input type="number" min={0} value={minSelect} onChange={e => setMinSelect(e.target.value)} className="h-9 text-sm" /></div>
        <div><label className="text-xs text-muted-foreground">Max Select</label>
          <Input type="number" min={1} value={maxSelect} onChange={e => setMaxSelect(e.target.value)} className="h-9 text-sm" /></div>
      </div>

      <p className="text-xs font-semibold text-muted-foreground">Modifiers</p>
      <div className="space-y-2">
        {rows.map((r, i) => (
          <div key={i} className="flex items-center gap-2 border border-border rounded-lg p-2">
            <Input value={r.name} onChange={e => updateRow(i, { name: e.target.value })} placeholder="Small" className="h-8 text-xs flex-1" />
            <Input type="number" step="0.01" value={r.priceDelta} onChange={e => updateRow(i, { priceDelta: e.target.value })} placeholder="+0.00" className="h-8 w-20 text-xs" />
            <label className="flex items-center gap-1 text-[11px] text-muted-foreground shrink-0">
              <input type="checkbox" checked={r.isActive} onChange={e => updateRow(i, { isActive: e.target.checked })} /> Active
            </label>
      ...
      <Button size="sm" variant="outline"
        ...
        <Plus className="w-3.5 h-3.5 mr-1" /> Add Modifier
      </Button>

      <Button className="w-full" disabled={!valid} onClick={handleSave}>Save Modifier Group</Button>
    </LeftDrawer>
  );
}
