...
const STATUS_META: Record<ReservationStatus, { label: string; color: string; bg: string }> = {
  ...
  seated:    { label: "Seated",    color: "text-success", bg: "bg-success/10" },
  completed: { label: "Completed", color: "text-muted-foreground", bg: "bg-muted/20" },
  cancelled: { label: "Cancelled", color: "text-destructive", bg: "bg-destructive/10" },
  no_show:   { label: "No-show",   color: "text-amber-500", bg: "bg-amber-500/10" },
};
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground flex items-center gap-2"><CalendarClock className="w-5 h-5" /> Reservations</h1>
          <p className="text-sm text-muted-foreground">Manage bookings and no-show policy.</p>
        </div>
        <div className="flex gap-2">
          <Can permission="restaurant.reservations.edit">
            <Button variant="outline" onClick={() => setShowRules(true)}><Settings className="w-4 h-4 mr-1" /> Policy</Button>
          </Can>
          <Can permission="restaurant.reservations.create">
            <Button onClick={() => setShowAdd(true)}><Plus className="w-4 h-4 mr-1" /> New Reservation</Button>
          </Can>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard label="Today's Covers" value={summary.todayCovers} />
          <StatCard label="Confirmed" value={summary.confirmed} accent="bg-primary" />
          <StatCard label="No-shows" value={summary.noShow} accent="bg-amber-500" />
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center text-muted-foreground text-sm">No reservations for this date.</div>
      ) : (
              ...
              <div key={r.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between gap-4">
                ...
                {r.status === "confirmed" && canEdit && (
                  <div className="flex gap-2 shrink-0">
                    <Button size="sm" onClick={() => seat.mutate(r.id)}><CheckCircle2 className="w-3.5 h-3.5 mr-1" /> Seat</Button>
      ...
      {showAdd && (
        <AddReservationModal tables={tables.filter(t => !t.mergedIntoTableId)} defaultDate={date} onClose={() => setShowAdd(false)} />
      )}
  ...
  return (
    <LeftDrawer onClose={onClose} widthClassName="max-w-md">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div><label className="text-xs text-muted-foreground">Email (optional)</label>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div><label className="text-xs text-muted-foreground">Time</label>
          <Input type="time" value={reservationTime} onChange={e => setReservationTime(e.target.value)} className="h-9 text-sm" /></div>
        <div><label className="text-xs text-muted-foreground">Covers</label>
          <Input type="number" min={1} value={covers} onChange={e => setCovers(Number(e.target.value))} className="h-9 text-sm" /></div>
      </div>
      {tables.length > 0 && (
        <div><label className="text-xs text-muted-foreground">Table (optional)</label>
          <select value={tableId} onChange={e => setTableId(e.target.value)}
            className="w-full h-9 text-sm rounded-md border border-border bg-card px-2">
            <option value="">Not assigned yet</option>
            {tables.map(t => <option key={t.id} value={t.id}>{t.tableNumber} ({t.capacity} seats)</option>)}
          </select></div>
      )}
      <div><label className="text-xs text-muted-foreground">Special Requests</label>
        <Input value={specialRequests} onChange={e => setSpecialRequests(e.target.value)} placeholder="Optional" className="h-9 text-sm" /></div>
      <Button className="w-full" disabled={!guestName.trim() || !guestPhone.trim() || createReservation.isPending}
        ...
        {createReservation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Plus className="w-4 h-4 mr-1" />} Create Reservation
      </Button>
    </LeftDrawer>
  );
}
...
function ReservationRuleModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <LeftDrawer onClose={onClose} widthClassName="max-w-md">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-foreground">Reservation Policy</p>
        <button onClick={onClose}><X className="w-4 h-4 text-muted-foreground" /></button>
      </div>
      <p className="text-xs text-muted-foreground">
        Auto-no-show flags a confirmed reservation once it's this many minutes past its time. Set to 0 to disable.
      </p>
      <div className="grid grid-cols-2 gap-2">
        <div><label className="text-xs text-muted-foreground">Slot Duration (min)</label>
        ...
        <div><label className="text-xs text-muted-foreground">Max Covers / Slot</label>
        ...
        <div><label className="text-xs text-muted-foreground">Max Advance (days)</label>
        ...
        <div><label className="text-xs text-muted-foreground">Min Notice (min)</label>
        ...
        <div><label className="text-xs text-muted-foreground">Auto No-Show (min)</label>
        ...
        <div><label className="text-xs text-muted-foreground">Deposit Amount</label>
      ...
      <Button className="w-full" disabled={saveRule.isPending}
        ...
        {saveRule.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : null} Save Policy
      </Button>
    </LeftDrawer>
  );
}
