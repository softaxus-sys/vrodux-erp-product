...
export function BranchAccessView() {
  ...
  if (!canEdit) {
    return <div className="p-6"><p className="text-sm text-muted-foreground">You don't have permission to manage branch access.</p></div>;
  }
    ...
    <div className="p-6 space-y-4">
      <div>
        <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
          <Building2 className="w-5 h-5 text-primary" /> Branch Access
        </h1>
        <p className="text-sm text-muted-foreground">
          Assign staff to specific branches — a user with no assignment sees every branch's tables, orders, reservations, and waitlist.
        </p>
      </div>

      {branchesLoading ? (
        <div className="flex items-center justify-center h-40 text-muted-foreground"><Loader2 className="animate-spin mr-2 h-5 w-5" /> Loading branches…</div>
      ) : branches.length === 0 ? (
        <p className="text-sm text-muted-foreground py-10 text-center">No branches configured for this tenant yet — add one in Settings → Branches first.</p>
      ) : (
...
function BranchRoster({ branchId, branchName, assignments }: { branchId: string; branchName: string; assignments: UserBranchDto[] }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 space-y-4">
      <p className="text-sm font-semibold text-foreground">{branchName} — Assigned Staff</p>

      {assignments.length === 0 ? (
        <p className="text-sm text-muted-foreground">No one is scoped to this branch yet.</p>
      ) : (
        <div className="space-y-1.5">
          {assignments.map(a => (
            <div key={a.id} className="flex items-center gap-2 bg-muted/20 border border-border rounded-lg px-3 py-2">
              ...
              <select value={a.role} onChange={e => updateRole.mutate({ id: a.id, role: e.target.value as BranchRole })}
                disabled={updateRole.isPending} className="h-8 text-xs rounded-md border border-border bg-card px-2 capitalize">
      ...
      <div className="pt-3 border-t border-border space-y-2">
        <div className="relative">
          ...
          <Input placeholder="Search users to assign…" value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9 text-sm" />
        </div>
