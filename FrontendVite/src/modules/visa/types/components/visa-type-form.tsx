        ...
        <>
          ...
          <motion.div className="fixed right-0 top-0 h-full w-full max-w-lg bg-card border-l border-border z-50 flex flex-col shadow-2xl"
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              <div>
                <h2 className="text-base font-bold">{isEdit ? "Edit Visa Type" : "New Visa Type"}</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Fees, processing time and the document checklist</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              <div className="space-y-1.5">
                ...
                <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Employment Visa — New" className="h-9 text-sm" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <select value={category} onChange={e => setCategory(e.target.value)}
                    ...
                    {VISA_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Processing Days</label>
                  <Input type="number" min={0} value={days} onChange={e => setDays(e.target.value)} className="h-9 text-sm" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Required documents (per applicant)</label>
                <div className="flex gap-2">
                  <Input value={newDoc} onChange={e => setNewDoc(e.target.value)} onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addDoc())}
                    placeholder="Add a document…" className="h-9 text-sm flex-1" />
                  <Button size="sm" className="h-9 gap-1" onClick={addDoc} disabled={!newDoc.trim()}><Plus className="h-3.5 w-3.5" />Add</Button>
                </div>
                <div className="space-y-1.5">
                  {docs.map((d, i) => (
                    <div key={`${d}-${i}`} className="flex items-center gap-2 bg-muted/30 rounded-lg px-3 py-2">
                      ...
                      <button onClick={() => setDocs(p => p.filter((_, idx) => idx !== i))}
                        className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive" aria-label="Remove">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                  ))}
                  {docs.length === 0 && <p className="text-xs text-muted-foreground">No documents yet — add the checklist items applicants must provide.</p>}
                </div>
              </div>
            </div>
