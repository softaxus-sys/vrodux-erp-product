...
const DOC_STATUS_META: Record<CaseDocumentStatus, { color: string; bg: string }> = {
  pending:  { color: "text-slate-600",   bg: "bg-slate-100 dark:bg-slate-800/50" },
  received: { color: "text-blue-600",    bg: "bg-blue-50 dark:bg-blue-900/20" },
  verified: { color: "text-success",     bg: "bg-success/10" },
  rejected: { color: "text-destructive", bg: "bg-destructive/10" },
  expired:  { color: "text-amber-600",   bg: "bg-amber-50 dark:bg-amber-900/20" },
};
            ...
            {isLoading || !c ? (
              ...
              <>
                ...
                <div className="flex items-start justify-between px-6 py-5 border-b border-border">
                  <div className="min-w-0 pr-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {meta && <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold", meta.color, meta.bg)}>{t(`cases.status.${c.status}`)}</span>}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1 font-mono">{c.caseNumber}{c.govtReference && <> · {t("drawer.govtRef")}: {c.govtReference}</>}</p>
                ...
                <div className="flex border-b border-border px-6">
                  {(["overview","applicants","documents","submissions","timeline"] as Tab[]).map(tabKey => (
                    <button key={tabKey} onClick={() => setTab(tabKey)}
                      ...
                      {t(`drawer.tab.${tabKey}`)}
                      {tabKey === "documents" && c.documents.length > 0 && (
                    ...
                    <>
                      {c.rejectionReason && (
                        <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-4">
                          <p className="text-sm font-semibold text-destructive mb-1">{t("drawer.rejected")}</p>
                          <p className="text-xs text-muted-foreground">{c.rejectionReason}</p>
                        </div>
                      )}
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-muted/30 rounded-xl p-3 text-center">
                          <p className="text-[10px] text-muted-foreground">{t("drawer.serviceFee")}</p>
                          <p className="font-bold text-sm">{formatCurrency(c.serviceFee, currency)}</p>
                        </div>
                        <div className="bg-muted/30 rounded-xl p-3 text-center">
                          <p className="text-[10px] text-muted-foreground">{t("drawer.govtFee")}</p>
                      ...
                      {/* Billing */}
                      <div>
                        <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">{t("drawer.billing")}</h4>
                        {c.invoiceNumber ? (
                          <Link to="/finance/invoicing" onClick={onClose}
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium group-hover:text-primary transition-colors">{t("drawer.invoiceNumber", { number: c.invoiceNumber })}</p>
                              <p className="text-[11px] text-muted-foreground">{t("drawer.viewInFinance", { amount: formatCurrency(c.serviceFee + c.govtFee, currency) })}</p>
                            </div>
                          ...
                          <Can permission="finance.invoicing.create"
                            fallback={<p className="text-xs text-muted-foreground">{t("drawer.noInvoice")}</p>}>
                            <Button variant="outline" size="sm" className="h-9 gap-1.5 w-full" disabled={genInvoice.isPending}
                              ...
                              {t("drawer.generateInvoice", { amount: formatCurrency(c.serviceFee + c.govtFee, currency) })}
                            </Button>
                          </Can>
                        )}
                      </div>
                      <div className="space-y-0 bg-muted/30 rounded-xl p-4">
                        {[
                          { key: "emirate",    label: t("drawer.field.emirate"),    value: c.emirate || "—" },
                          { key: "channel",    label: t("drawer.field.channel"),    value: c.channel },
                          { key: "priority",   label: t("drawer.field.priority"),   value: t(`wizard.priorityValue.${c.priority}`, { defaultValue: c.priority }) },
                          { key: "assignedTo", label: t("drawer.field.assignedTo"), value: c.assignedTo || "—" },
                          { key: "slaDue",     label: t("drawer.field.slaDue"),     value: c.slaDueDate ? formatDate(c.slaDueDate, "medium") : "—" },
                          ...(c.visaExpiryDate ? [{ key: "visaExpiry", label: t("drawer.field.visaExpiry"), value: formatDate(c.visaExpiryDate, "medium") }] : []),
                          { key: "created",    label: t("drawer.field.created"),    value: formatDate(c.createdAt, "medium") },
                        ].map(row => (
                          <div key={row.key} className="flex justify-between gap-4 py-2 border-b border-border/40 last:border-0">
                        ...
                        <div key={a.id} className="flex items-start gap-3 p-4 bg-muted/30 rounded-xl">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-primary/10 text-primary">{t(`wizard.relationship.${a.relationship}`, { defaultValue: a.relationship })}</span>
                            </div>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs text-muted-foreground">
                              <span>{t("drawer.applicant.passport")}: <span className="font-mono text-foreground">{a.passportNumber}</span></span>
                              <span>{t("drawer.applicant.nationality")}: {a.nationality}</span>
                              {a.passportExpiry && <span>{t("drawer.applicant.expiry")}: {formatDate(a.passportExpiry, "medium")}</span>}
                              {a.dateOfBirth && <span>{t("drawer.applicant.dob")}: {formatDate(a.dateOfBirth, "medium")}</span>}
                              {a.emiratesId && <span>{t("drawer.applicant.eid")}: {a.emiratesId}</span>}
                              {a.uidNumber && <span>{t("drawer.applicant.uid")}: {a.uidNumber}</span>}
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  {tab === "documents" && (
                    <div className="space-y-3">
                      <Can permission="visa.cases.edit">
                        <div className="flex gap-2">
                          <Input value={newDocName} onChange={e => setNewDocName(e.target.value)}
                            ...
                            placeholder={t("drawer.addDocPlaceholder")} className="h-8 text-sm flex-1" />
                          <Button size="sm" className="h-8 gap-1" disabled={!newDocName.trim() || addDoc.isPending} onClick={submitNewDoc}>
                            <Plus className="h-3.5 w-3.5" />{t("drawer.button.add")}
                          </Button>
                        </div>
                      </Can>
                      {c.documents.map(d => {
                          ...
                          <div key={d.id} className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[11px] text-muted-foreground">
                                {applicant ? applicant.fullName : t("drawer.caseLevel")}
                                {d.expiryDate && <> · {t("drawer.expires")} {formatDate(d.expiryDate, "medium")}</>}
                                {d.notes && <> · {d.notes}</>}
                              </p>
                            </div>
                            <Can permission="visa.cases.edit"
                              fallback={<span className={cn("px-2 py-0.5 rounded-full text-[11px] font-semibold", dm.color, dm.bg)}>{t(`drawer.docStatus.${d.status}`)}</span>}>
                              <select value={d.status} disabled={updateDoc.isPending}
                                ...
                                {DOC_STATUSES.map(s => <option key={s} value={s}>{t(`drawer.docStatus.${s}`)}</option>)}
                              </select>
                            </Can>
                          </div>
                        );
                      ...
                      {c.documents.length === 0 && (
                        <p className="text-xs text-muted-foreground text-center py-6">{t("drawer.noDocuments")}</p>
                      )}
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      <div className="flex gap-2">
                        <Input value={note} onChange={e => setNote(e.target.value)} onKeyDown={e => e.key === "Enter" && submitNote()}
                          placeholder={t("drawer.notePlaceholder")} className="h-8 text-sm flex-1" />
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium leading-tight">
                                {e.eventType === "status_change" && e.fromStatus && e.toStatus
                                  ? `${t(`cases.status.${e.fromStatus}`, { defaultValue: e.fromStatus })} → ${t(`cases.status.${e.toStatus}`, { defaultValue: e.toStatus })}`
                                  : e.note ?? e.eventType}
                              </p>
                ...
                <div className="border-t border-border px-6 py-4 flex items-center gap-2 flex-wrap">
                  ...
                  <Can permission="visa.cases.delete">
                    <Button variant="ghost" size="sm" className="gap-1.5 h-9 text-destructive hover:bg-destructive/5 ml-auto" disabled={busy}
                      ...
                      <Trash2 className="h-3.5 w-3.5" />{t("drawer.button.delete")}
                    </Button>
                  </Can>
                </div>
                    ...
                    <>
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
                        ...
                        <p className="font-semibold text-sm">
                          {pendingStatus === "submitted" ? t("drawer.prompt.submittedTitle") : pendingStatus === "issued" ? t("drawer.prompt.issuedTitle") : t("drawer.prompt.rejectedTitle")}
                        </p>
                        <p className="text-xs text-muted-foreground mt-1 mb-3">
                          {pendingStatus === "submitted"
                            ? t("drawer.prompt.submittedHint")
                            : pendingStatus === "issued"
                            ? t("drawer.prompt.issuedHint")
                            : t("drawer.prompt.rejectedHint")}
                        </p>
                        <Input
                          type={pendingStatus === "issued" ? "date" : "text"}
                          value={statusExtra} onChange={e => setStatusExtra(e.target.value)}
                          placeholder={pendingStatus === "submitted" ? t("drawer.prompt.submittedPlaceholder") : t("drawer.prompt.rejectedPlaceholder")}
                          className="h-9 text-sm" />
                        <div className="flex justify-end gap-2 mt-4">
                          <Button variant="outline" size="sm" onClick={() => setPendingStatus(null)} disabled={changeStatus.isPending}>{t("drawer.button.cancel")}</Button>
                          <Button size="sm" className={cn("gap-1.5", pendingStatus === "rejected" && "bg-destructive hover:bg-destructive/90")}
                            ...
                            {pendingStatus === "submitted" ? t("drawer.prompt.submitConfirm") : pendingStatus === "issued" ? t("drawer.prompt.issueConfirm") : t("drawer.prompt.rejectConfirm")}
                          </Button>
                        </div>
                      </motion.div>
                    </>
                    ...
                    <>
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {t("drawer.deleteBodyPrefix")} <span className="font-medium text-foreground">{c.caseNumber}</span> {t("drawer.deleteBodySuffix")}
                        </p>
                        <div className="flex justify-end gap-2 mt-4">
                          <Button variant="outline" size="sm" onClick={() => setConfirmDelete(false)} disabled={del.isPending}>{t("drawer.button.cancel")}</Button>
                          <Button size="sm" className="bg-destructive hover:bg-destructive/90 gap-1.5" disabled={del.isPending}
                            ...
                            {del.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}{t("drawer.button.delete")}
                          </Button>
                        </div>
                      </motion.div>
                    </>
...
function CaseSubmissionsPanel({ caseId, defaultChannel, byName }: { caseId: string; defaultChannel: string; byName: string }) {
  ...
  // Backend-driven enum values: translate, falling back to a de-underscored label.
  const typeLabel   = (s: string) => t(`drawer.submissions.type.${s}`,   { defaultValue: s.replace(/_/g, " ") });
  const statusLabel = (s: string) => t(`drawer.submissions.status.${s}`, { defaultValue: s.replace(/_/g, " ") });

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t("drawer.submissions.title")}</h4>
        <Can permission="visa.cases.edit">
          <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5" onClick={() => setAdding(v => !v)}>
            <Plus className="h-3.5 w-3.5" />{t("drawer.submissions.record")}
          </Button>
        </Can>
      </div>
        ...
        <div className="rounded-xl border border-border p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            ...
            <select value={type} onChange={e => setType(e.target.value)}
              className="h-9 px-2 rounded-lg border border-border bg-card text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30">
              {SUBMISSION_TYPES.map(st => <option key={st} value={st}>{typeLabel(st)}</option>)}
            </select>
          </div>
          <div className="flex gap-2">
            <Input value={ref} onChange={e => setRef(e.target.value)} placeholder={t("drawer.submissions.refPlaceholder")} className="h-9 text-sm flex-1" />
            <Button size="sm" className="h-9" onClick={submit} disabled={create.isPending}>
              {create.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : t("drawer.button.add")}
            </Button>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted-foreground text-center py-6">{t("drawer.submissions.empty")}</p>
      ) : subs.map(s => (
        <div key={s.id} className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">{typeLabel(s.submissionType)}</p>
            <p className="text-[11px] text-muted-foreground">
              <span className="uppercase">{s.channel}</span>{s.externalReference && <> · {s.externalReference}</>} · {formatDate(s.submittedAt, "medium")}
            </p>
          </div>
          <Can permission="visa.cases.edit"
            fallback={<span className={cn("px-2 py-0.5 rounded-full text-[11px] font-semibold", SUB_STATUS_CLS[s.status] ?? "bg-muted")}>{statusLabel(s.status)}</span>}>
            <select value={s.status} onChange={e => caseId && update.mutate({ caseId, submissionId: s.id, status: e.target.value })}
              className="h-8 px-2 rounded-lg border border-border bg-card text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30">
              {SUBMISSION_STATUSES.map(st => <option key={st} value={st}>{statusLabel(st)}</option>)}
            </select>
          </Can>
        </div>
      ))}
