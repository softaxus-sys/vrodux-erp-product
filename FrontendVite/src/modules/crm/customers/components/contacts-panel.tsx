  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        {editingId === null && <Button size="sm" className="h-8 text-xs gap-1.5" onClick={startAdd}><Plus className="h-3.5 w-3.5" />Add Contact</Button>}
      </div>
      ...
      {contacts.length === 0 && editingId === null && (
        <p className="text-xs text-muted-foreground text-center py-6">No contacts yet. Add the people you work with at this account.</p>
      )}
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {c.isPrimary && <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-primary bg-primary/10 px-1.5 py-0.5 rounded-full"><Star className="h-2.5 w-2.5" />Primary</span>}
              </div>
            ...
            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              {!c.isPrimary && <button title="Set primary" onClick={() => setPrimary.mutate(c.id)} className="p-1.5 rounded text-muted-foreground hover:text-primary"><Star className="h-3.5 w-3.5" /></button>}
              <button title="Edit" onClick={() => startEdit(c)} className="p-1.5 rounded text-muted-foreground hover:text-foreground"><Pencil className="h-3.5 w-3.5" /></button>
              <button title="Remove" onClick={() => del.mutate(c.id)} className="p-1.5 rounded text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
          </div>
        </div>
      ))}
...
function ContactForm({ form, setForm, onSave, onCancel, saving }: {
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold">{form.firstName ? "Edit contact" : "New contact"}</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Input value={form.firstName} onChange={e => set("firstName", e.target.value)} placeholder="First name *" className="h-8 text-sm" />
        <Input value={form.lastName} onChange={e => set("lastName", e.target.value)} placeholder="Last name" className="h-8 text-sm" />
        <Input value={form.title} onChange={e => set("title", e.target.value)} placeholder="Job title" className="h-8 text-sm" />
        <Input value={form.department ?? ""} onChange={e => set("department", e.target.value)} placeholder="Department" className="h-8 text-sm" />
        <Input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="Email" className="h-8 text-sm" />
        <Input value={form.phone} onChange={e => set("phone", e.target.value)} placeholder="Phone" className="h-8 text-sm" />
      </div>
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" size="sm" className="h-7 text-xs" onClick={onCancel} disabled={saving}>Cancel</Button>
        <Button size="sm" className="h-7 text-xs" onClick={onSave} disabled={saving || !form.firstName.trim()}>{saving ? "Saving…" : "Save"}</Button>
      </div>
    </div>
  );
}
